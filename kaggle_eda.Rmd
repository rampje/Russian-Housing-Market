...
xgb_train_df <- train %>% select(-id)
xgb_test_df <- test %>% select(-id)

xgb_train_df <- full_join(xgb_train_df, macro_preds, by="timestamp") %>%
                  select(-timestamp)
xgb_test_df <- full_join(xgb_test_df, macro_preds, by="timestamp") %>%
                  select(-timestamp)

xgb_train_price <- xgb_train_df$price_doc
xgb_train_df <- xgb_train_df %>% select(-price_doc) %>% as.matrix
xgb_train_df <- as(xgb_train_df, "dgCMatrix")

#xgb_test_df <- as.matrix(data.frame(xgb_test_df))
#xgb_test_df <- as(xgb_test_df, "dgCMatrix")

#boost1 <- xgboost(data = xgb_train_df, label = xgb_train_price, 
#                 max_depth = 6, eta = 0.1, nrounds = 10)
